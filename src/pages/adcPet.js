...
function AdicionarPet() {
  ...
  const handleAdicionarPet = async () => {
    if (nome && idade && pelagem && peso && tamanho && usuarioID && valorBanho) {
      try {
        await addDoc(collection(db, 'Pets'), {
          ...
          PróximaVacinação: proximaVacinacao ? new Date(proximaVacinacao) : null,
          UsuarioID: usuarioID,
          ValorBanho: parseFloat(valorBanho),
        });
    ...
    <div>
      ...
      <form noValidate>
        ...
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label="Usuário" fullWidth margin="normal" />}
        />
        <TextField
