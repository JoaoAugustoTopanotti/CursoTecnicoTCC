      ...
      try {
        ...
        const agendamentosData = await Promise.all(
          querySnapshot.docs.map(async docSnap => {
            ...
            const petData = petDocSnap.exists()
              ? petDocSnap.data()
              : { Nome: 'Desconhecido' }
        ...
        const próxima = updatedAgendamentos
          ...
          .reduce(
            ...
            { agendamento: null, diff: Number.POSITIVE_INFINITY }
          )
  ...
  const handleDateChange = async date => {
    ...
    try {
      if (isToday(date)) {
        ...
        const agendamentosPendentes = filteredData.filter(
          agendamento => agendamento.Status === 'Pendente'
        )
        const agendamentosFinalizados = filteredData.filter(
          agendamento => agendamento.Status === 'Finalizado'
        )
    ...
    <div className={styles.visualizarAgenda}>
      ...
      {nextAppointment && (
        <div className={styles.notification}>
          ...
          {Math.ceil(
            (nextAppointment.DataHora.toDate() - new Date()) / (1000 * 60 * 60)
          )}{' '}
          horas.
        </div>
      )}
